...
const order = new OrderController(orderService, cartService);

// 1. PEDIDOS
// =========================================================

// Obtener pedidos de un local
router.get("/locals/:id/orders", order.getOrders.bind(order));

// Obtener pedidos de un usuario
router.get("/user/orders", isAuthenticated, order.getUserOrders.bind(order));

// Obtener orden por ID
router.get("/user/orders/:id", order.getById.bind(order));
...
router.post(
  "/locals/:id/orders/manual",
  isAuthenticated,
  order.createManualOrder.bind(order),
);

// Actualizar estado de una orden (staff)
router.patch(
  "/locals/:id/orders/:orderId/status",
  isAuthenticated,
  order.updateOrderStatus.bind(order),
);

// Editar items de una orden (staff)
router.put(
  "/locals/:id/orders/:orderId/items",
  isAuthenticated,
  order.updateOrderItems.bind(order),
);
